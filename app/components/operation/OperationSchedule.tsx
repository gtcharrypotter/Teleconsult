...
const OperationSchedule = ({ appointment, patient }) => {
  const { user } = useAuth()
  ...
  return (
    <View className="flex flex-col gap-8">
      <View className="flex flex-row gap-4">
        <Text className="text-3xl font-psemibold">Operation Schedule</Text>
        {user?.specialty?.name === 'Anesthesiologist' && (
          <View>
            <TouchableOpacity
              onPress={() => operationSchedRef.current?.show(patient, appointment)}
              activeOpacity={0.7}
            >
      ...
      <View className="flex-col justify-center items-center gap-8 mt-12">
        <Text className="text-3xl font-psemibold">
          Remaining Time Before Operation Start
        </Text>
        <View className="flex-row">
          <Text className="text-4xl font-pextrabold text-red-700">
            {timeLeft}
          </Text>
        </View>
      </View>
