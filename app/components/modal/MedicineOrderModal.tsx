...
const MedicineOrderModal = forwardRef((props, ref) => {
  ...
  if (!selectedItems || !quantity || parseInt(quantity) <= 0) {
  ...
  try {
    ...
    formData.append("inventory_id[]", selectedItems?.inventory?.id); 
    formData.append("items[]", selectedItems?.id);
    formData.append("quantity[]", quantity);
    formData.append("sig[]", sig); 
    console.log("Data:", formData);

    const response = await Axios.post(`/v1/hospital/admitted-medicines/${appointment?.id}`, formData, {
        headers: {
